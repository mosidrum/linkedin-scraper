...
export function normalizeProfile(raw: Raw, profileUrl: string, provider: string): LinkedInProfile {
  ...
  return {
    ...
    avatarUrl: str(raw.profileImg ?? raw.imgUrl ?? raw.profileImageUrl ?? (typeof raw.profilePicture === 'string' ? raw.profilePicture : raw.profilePicture?.url) ?? raw.photoUrl ?? raw.avatar ?? raw.profilePictureUrl) || null,
    bannerUrl: str(raw.backgroundImg ?? raw.backgroundImgUrl ?? raw.backgroundImage?.[0]?.url ?? (typeof raw.backgroundImage === 'string' ? raw.backgroundImage : null) ?? raw.backgroundCoverImageUrl ?? raw.bannerUrl) || null,
    ...
    recommendationsCount: num(raw.recommendationsCount ?? raw.numberOfRecommendationsReceived ?? (Array.isArray(raw.recommendations) ? raw.recommendations.length : null) ?? raw.recommendationsReceived),
